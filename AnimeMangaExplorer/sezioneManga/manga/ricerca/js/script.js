...
async function searchMedia(type){
    ...
    try {
        ...
        if(!data.data || data.data.length === 0){
            ...
            mainContent.classList.add('no-results');  // Aggiungi la classe per lo sfondo
            return;
        }

        resultsContainer.innerHTML = '';  // Pulisce i risultati precedenti
        mainContent.classList.remove('no-results');  // Rimuovi la classe quando ci sono risultati
...
document.getElementById('search-manga-button').addEventListener('click', () => searchMedia('manga'));